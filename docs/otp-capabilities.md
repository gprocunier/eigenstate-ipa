...
flowchart TD
    ...
    new_token --> add_totp["add, type=totp/hotp"]
    enroll_pw --> add_host["add, type=host"]
...
flowchart TD
    ...
    type_q -->|"User 2FA"| add_totp["add, type=totp or hotp"]
    type_q -->|"Host enrollment"| add_host["add, type=host"]
...
Use `operation=add` with `type=totp` (default) to create a new 2FA token for
...
Use `operation=add`, `type=host` to generate a one-time enrollment password
for a host, then pass it to `freeipa.ansible_freeipa.ipaclient`.

```mermaid
flowchart TD
    ipahost["freeipa.ansible_freeipa.ipahost\ncreate host record in IdM"] --> enroll_pw["otp add, type=host\nFQDN=web-01.example.com"]
...
The host record must already exist in IdM before calling `type=host`. The
`ipahost` module creates the record; `otp add type=host` then sets the
...
Loop `operation=add`, `type=host` over an inventory group to generate
...
```mermaid
flowchart TD
    group["inventory group\nnew_hosts"] --> loop["loop: otp add type=host\nfor each FQDN"]
