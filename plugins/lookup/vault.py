...
options:
  ...
  verify:
    description: >-
      Path to the IPA CA certificate for TLS verification. If omitted,
      the plugin tries C(/etc/ipa/ca.crt). If no CA path is available, the
      lookup fails unless C(verify) is set to C(false) explicitly.
    type: str
    env:
      - name: IPA_CERT
  username:
